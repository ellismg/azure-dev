				...
				DependsOn: []*armresources.BasicDependency{
					{
						...
						ResourceName: to.Ptr("RESOURCE_GROUP"),
						ResourceType: to.Ptr(string(infra.AzureResourceTypeResourceGroup)),
					},
				},
			},
		},
...
func prepareStateMocks(mockContext *mocks.MockContext) {
	...
	mockContext.HttpClient.When(func(request *http.Request) bool {
		return request.Method == http.MethodGet && strings.HasSuffix(
			request.URL.Path,
			"/SUBSCRIPTION_ID/providers/Microsoft.Resources/deployments/test-env",
		)
